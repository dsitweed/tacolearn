...
export function ForSchoolsSection() {
  ...
  return (
    <section
      id="for-schools"
      className="relative overflow-hidden bg-slate-900 py-16 text-white lg:py-24"
    >
      {/* Background glow */}
      <div className="pointer-events-none absolute top-1/2 left-0 size-96 -translate-y-1/2 rounded-full bg-indigo-600/10 blur-3xl" />
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12">
          ...
          <div className="space-y-6 lg:col-span-6">
            <Badge
              variant="outline"
              className="rounded-full border-indigo-500/30 bg-indigo-500/10 px-3 py-1 text-xs font-semibold text-indigo-300"
            >
              ...
              {benefits.map((item) => {
                ...
                return (
                  <div key={item.title} className="flex items-start gap-3.5">
                    <div className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-lg border border-indigo-500/30 bg-indigo-600/20 text-indigo-400">
            ...
            <div className="pt-4">
              <Button
                size="lg"
                className="h-12 rounded-xl bg-indigo-600 px-6 text-sm font-semibold text-white shadow-lg shadow-indigo-600/20 hover:bg-indigo-700"
                asChild
              >
                <Link href="#contact">
                  Liên hệ hợp tác trường học
